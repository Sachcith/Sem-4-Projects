...
T zcr(Matrix<T,N,1> &input,std::size_t NUM_FRAMES,std::size_t NUM_BINS){
    ...
    YIELD(); for(std::size_t i=0;i<NUM_FRAMES;i++){
        YIELD(); for(std::size_t j=2;j<NUM_BINS;j++){
...
class spectral_contrast{
    ...
    public:
        Matrix<T,4,1> compute(Matrix<T,N,1> &input, Matrix<T,NUM_FRAMES,1> &temp){
            ...
            YIELD(); for(std::size_t k=0;k<n_bands;k++){
                YIELD(); for(std::size_t i=0;i<NUM_FRAMES;i++){
                    ...
                    YIELD(); for(std::size_t j=k*band_size;j<end;j++){
...
class temporal_features{
    public:
        Matrix<T,5,1> compute(Matrix<T,N,1> &input, Matrix<T,NUM_FRAMES,1> frame_energy, Matrix<T,NUM_FRAMES-1,1> onset_strength){
            ...
            YIELD(); for(std::size_t i=1;i<NUM_FRAMES;i++){
...
class MFCC{
    public:
        ...
        Matrix<T,2,1> compute(Matrix<T,N,1> &input, std::size_t sr){ // sr = sample rate
            ...
            // YIELD(); for(std::size_t i=0;i<N;i++) input[i][0] = std::log10(std::abs(input[i][0])+1e-10);
            input.reshape(NUM_FRAMES,NUM_BINS);
            // YIELD(); for(std::size_t t = 0; t < NUM_FRAMES; t++){
            //     YIELD(); for(std::size_t c = 0; c < 13; c++){
            //         mfcc[c][t] = 0;
            //         YIELD(); for(std::size_t m = 0; m < NUM_BINS; m++){
            //             mfcc[c][t] += input[t][m] *
            //                         std::cos(M_PI * c * (m + 0.5) / NUM_BINS);}
            //     }
            // }
            YIELD(); for(size_t c = 0; c < 13; c++){
                YIELD(); for(size_t t = 0; t < NUM_FRAMES; t++){
                    double sum = 0;
                    YIELD(); for(size_t m = 0; m < NUM_BINS; m++){
                        sum += input[t][m] *
