...
class Dense{
    public:
        Matrix<T,IN,OUT> weight; // size = output * input
        Matrix<T,OUT,1> bias; // size = output * 1
    public:
        void setWeight(Matrix<T,IN,OUT> weight_pointer);
        void setBias(Matrix<T,OUT,1>);
        Matrix<T,OUT,1> forward(Matrix<T,IN,1> input);
};

template <class T, std::size_t OUT, std::size_t IN>
void Dense<T,OUT,IN>::setWeight(Matrix<T,IN,OUT> weight_input){
    for(int i=0;i<OUT;i++){
        for(int j=0;j<IN;j++){
            weight[j][i] = weight_input[j][i];
        }
    }
}
...
Matrix<T,OUT,1> Dense<T,OUT,IN>::forward(Matrix<T,IN,1> input){
    ...
    for(int i=0;i<OUT;i++){
        output[i][0] = 0;
        for(int k=0;k<IN;k++){
            output[i][0] = output[i][0] + weight[k][i]*input[k][0];
        }
