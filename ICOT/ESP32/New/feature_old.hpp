...
class MFCC{
    public:
        ...
        Matrix<T,2,1> compute(Matrix<T,N,1> &input, std::size_t sr){ // sr = sample rate
            ...
            //     bin[i][0] = std::floor(((NUM_BINS-1)*2 + 1) * f / sr);
            ...
            //     start = std::min(start,NUM_BINS-1);
            //     center = std::min(center,NUM_BINS-1);
            //     end = std::min(end,NUM_BINS-1);
            ...
            //         if(mel_energy[i][j] + 1e-10 > 0) mel_energy[i][j] = std::log(mel_energy[i][j] + 1e-10);
            //         else mel_energy[i][j] = 0;
            //     }
            // }
            ...
            //     }
            // }
            for(std::size_t i=0;i<N;i++) input[i][0] = std::log(std::abs(input[i][0]+1e-10));
            for(std::size_t t = 0; t < NUM_FRAMES; t++){
                for(std::size_t c = 0; c < 13; c++){
                    mfcc[c][t] = 0;
