...
class dwt{
    ...
    public:
        ...
        Matrix<T,final_size,1> compute(Matrix<T,final_size,1> &signal){
            ...
            signal.reset_shape();
            for(std::size_t i=0;i<signal.rows();i++) signal[i][0] = std::abs(signal[i][0]);
            return signal;
        }
        ...
        Matrix<T,Output_row*Output_col,1> zoom(Matrix<T,N,1> &input){
            ...
            for(std::size_t i=0;i<Output_row;i++){
                for(std::size_t j=0;j<Output_col;j++){
                    ...
                    // if(r < 0) r = 0;
                    // if(r > Input_row - 1) r = Input_row - 1;
                    ...
                    // if(c < 0) c = 0;
                    // if(c > Input_col - 1) c = Input_col - 1;
                    ...
                    if(r0 < 0) r0 = 0;
                    if(r1 < 0) r1 = 0;
                    if(r0 >= (int)Input_row) r0 = Input_row - 1;
                    if(r1 >= (int)Input_row) r1 = Input_row - 1;

                    T c = (j + 0.5) * col_scale - 0.5;

                    int c0_raw = std::floor(c);
                    int c1_raw = c0_raw + 1;
                    ...
                    if(c0 < 0) c0 = 0;
                    if(c1 < 0) c1 = 0;
                    if(c0 >= (int)Input_col) c0 = Input_col - 1;
                    if(c1 >= (int)Input_col) c1 = Input_col - 1;                
