...
class dwt{
    ...
    public:
        ...
        Matrix<T,final_size,1> compute(int level){
            stop_level = level;
...
template <class T, std::size_t TOTAL_SAMPLES,std::size_t max_level>
void dwt<T,TOTAL_SAMPLES,max_level>::compute_dwt(int level,size_t input){
    ...
    for(int i=-shift;i<(int)input-shift;i=i+2){
        for(int j=0;j<wavelet_size;j++){
            if(i+j<0) A[(i+shift)/2][0] = A[(i+shift)/2][0] + filter_l[j]*signal[-(i+j)-1][0];
            else if(i+j<input) A[(i+shift)/2][0] = A[(i+shift)/2][0] + filter_l[j]*signal[i+j][0];
            else A[(i+shift)/2][0] = A[(i+shift)/2][0] + filter_l[j]*signal[2*input-1-(i+j)][0];

            if(i+j<0) D[(i+shift)/2][0] = D[(i+shift)/2][0] + filter_h[j]*signal[-(i+j)-1][0];
            else if(i+j<input) D[(i+shift)/2][0] = D[(i+shift)/2][0] + filter_h[j]*signal[i+j][0];
            else D[(i+shift)/2][0] = D[(i+shift)/2][0] + filter_h[j]*signal[2*input-1-(i+j)][0];
        }
    }
    for(size_t i=0;i<input/2;i++){
        signal[i][0] = A[i][0];
